...
export const palettes = {
  signal: { paper: "#dcfa52", ink: "#171713", accent: "#ef4c33" },
  broadcast: { paper: "#ef4c33", ink: "#171713", accent: "#f3f0e8" },
  press: { paper: "#f3f0e8", ink: "#171713", accent: "#dcfa52" },
};
const fontsPromise = Promise.all(
  ["Anton.ttf", "BarlowCondensed.ttf", "BarlowCondensed-Bold.ttf"].map((f) =>
    readFile(join(FONT, f)),
  ),
);
const fontNames = ["Anton", "Barlow Condensed", "Barlow Condensed Bold"];
const rendererFingerprint = Promise.all([
  readFile(new URL(import.meta.url)),
  readFile(new URL("./project.mjs", import.meta.url)),
  fontsPromise,
]).then(([renderer, project, fonts]) =>
  digest([
    renderer.toString(),
    project.toString(),
    ...fonts.map((font) => font.toString("base64")),
  ]),
);
export async function artifactRevision(p) {
  return digest({
    project: publicProject(p),
    renderer: await rendererFingerprint,
  });
}
export async function fontCSS() {
  ...
  return (
    fonts
      .map(
        (f, i) =>
          `@font-face{font-family:'${fontNames[i]}';src:url(data:font/ttf;base64,${f.toString("base64")}) format('truetype');font-display:swap}`,
      )
      .join("") +
    `@font-face{font-family:'Barlow Condensed';font-weight:700;src:url(data:font/ttf;base64,${fonts[2].toString("base64")}) format('truetype')}`
  );
}
...
function wrap(doc, text, size, width, font) {
  ...
  for (const paragraph of text.split("\n")) {
    for (const word of paragraph.split(/\s+/).filter(Boolean)) {
      if (doc.widthOfString((line ? line + " " : "") + word) <= width) {
        line += (line ? " " : "") + word;
        continue;
      }
      if (line) {
      ...
      if (doc.widthOfString(word) > width) {
        for (const ch of word) {
          if (doc.widthOfString(line + ch) > width) {
            lines.push(line);
            line = "";
          }
          line += ch;
...
export async function posterSVG(p, url = "", shape = "letter") {
  ...
  measure
    .registerFont("Anton", anton)
    .registerFont("Barlow Condensed", barlow)
    .registerFont("Barlow Condensed Bold", bold);
  const W = shape === "square" ? 1080 : shape === "story" ? 1080 : 816,
    H =
      shape === "square"
        ? 1080
        : shape === "story"
          ...
          : shape === "a4"
            ? 1154
            : 1056;
  const { paper, ink } = palettes[p.theme],
    m = W * 0.055,
    inner = W - 2 * m;
  let parts = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><style>${await fontCSS()}</style><rect width="${W}" height="${H}" fill="${paper}"/>`,
  ];
  function block(
    text,
    y,
    ...
    do {
      lines = wrap(measure, text, size, lineWidth, font);
      if (lines.length * size * 1.15 <= h) break;
      size -= 1;
    } while (size > 9);
    const top = y + (h - lines.length * size * 1.15) / 2;
    lines.forEach((l, i) =>
      parts.push(
        `<text x="${W / 2}" y="${top + (i + 0.85) * size * 1.15}" text-anchor="middle" font-family="${font === "Barlow Condensed Bold" ? "Barlow Condensed" : font}" font-weight="${font === "Barlow Condensed Bold" ? 700 : 400}" font-size="${size}" fill="${ink}">${esc(l)}</text>`,
      ),
    );
  }
  let top = m;
  ...
  block(
    p.title || "YOUR VOICE BELONGS HERE",
    top,
    area * 0.48,
    W * 0.23,
    "Anton",
  ...
  parts.push(
    `<rect x="${m}" y="${top + area * 0.5}" width="${inner}" height="${Math.min(18, H * 0.014)}" fill="${ink}"/>`,
  );
  block(
    `${displayDate(p)} / ${displayTime(p)}`,
    top + area * 0.54,
    area * 0.09,
    W * 0.052,
    "Anton",
  ...
  block(
    p.address || "Add the full address",
    top + area * 0.74,
    area * 0.055,
    W * 0.032,
  ...
  block(
    p.demand || p.reason || "One clear reason. One concrete request.",
    top + area * 0.8,
    area * 0.12,
    W * 0.043,
    "Barlow Condensed Bold",
  ...
  block(
    p.organizer || "Your organizer name",
    top + area * 0.93,
    area * 0.065,
    W * 0.034,
    "Barlow Condensed",
  ...
  if (url) {
    ...
    const vb = qr.match(/viewBox="([^"]+)"/)[1];
    const content = qr
      .replace(/^.*?<svg[^>]*>/s, "")
      .replace(/<\/svg>\s*$/, "");
    const q = W * 0.2;
    parts.push(
      `<svg x="${m}" y="${bottom - q}" width="${q}" height="${q}" viewBox="${vb}">${content}</svg>`,
    );
    parts.push(
      `<text x="${m + q + 20}" y="${bottom - q + 40}" font-family="Barlow Condensed" font-weight="700" font-size="${W * 0.037}" fill="${ink}">SCAN FOR DETAILS &amp; UPDATES</text>`,
    );
    const short = url.replace("https://", "");
    const sz = Math.min(22, ((inner - q - 20) / short.length) * 1.8);
    parts.push(
...
export async function posterPNG(p, url = "", shape = "square") {
  const svg = await posterSVG(p, url, shape);
...
export async function posterPDF(p, url = "") {
  const svg = await posterSVG(p, url, p.format),
    fonts = await fontsPromise;
  const size = p.format === "a4" ? [595.28, 841.89] : [612, 792];
  const doc = new PDFDocument({
    size,
    margin: 0,
    info: { Title: p.title || "Protest flier", Creator: "Protest" },
  });
  fonts.forEach((f, i) => doc.registerFont(fontNames[i], f));
  const chunks = [];
...
export function calendar(p, url = "") {
  const { start, end } = timing(p);
  const stamp = (z) =>
    z.toInstant().toString({ smallestUnit: "second" }).replace(/[-:]/g, "");
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Protest//Event//EN",
    "CALSCALE:GREGORIAN",
    "BEGIN:VEVENT",
    `UID:${p.id}@protest.local`,
    `DTSTAMP:${new Date()
      .toISOString()
      .replace(/[-:]/g, "")
      .replace(/\.\d{3}/, "")}`,
    ...
    `DESCRIPTION:${icsEscape(p.reason + "\n" + p.demand + "\n" + p.meetingPoint)}`,
    `LOCATION:${icsEscape(p.location + ", " + p.address)}`,
    ...(url ? [`URL:${url}`] : []),
    `STATUS:${p.status === "cancelled" ? "CANCELLED" : p.status === "postponed" ? "TENTATIVE" : "CONFIRMED"}`,
    "END:VEVENT",
    "END:VCALENDAR",
  ];
  return (
    lines
      .map((line) => {
        let out = "",
          part = "";
        for (const c of line) {
          if (Buffer.byteLength(part + c) > 73) {
            out += part + "\r\n ";
            part = "";
          }
          part += c;
...
export async function siteHTML(project, url = "", revision = "preview") {
  const p = publicProject(project),
    ...
    pal = palettes[p.theme];
  const section = (id, title, body) =>
    body
      ? `<section class="section" id="${id}"><h2>${title}</h2><div><p>${esc(body)}</p></div></section>`
      : "";
  const schema = {
    ...
    location: { "@type": "Place", name: p.location, address: p.address },
    organizer: { "@type": "Organization", name: p.organizer },
    eventStatus: `https://schema.org/${p.status === "cancelled" ? "EventCancelled" : p.status === "postponed" ? "EventPostponed" : "EventScheduled"}`,
  };
  try {
    const t = timing(p);
    schema.startDate = t.start.toString({ timeZoneName: "never" });
    if (t.end) schema.endDate = t.end.toString({ timeZoneName: "never" });
  } catch {}
  const meta = url
    ? `<link rel="canonical" href="${esc(url)}"><meta property="og:url" content="${esc(url)}"><meta property="og:image" content="${esc(url)}social.png"><meta name="twitter:card" content="summary_large_image">`
    : "";
  ...
  ${
    p.sources.length
      ? `<section class="section" id="background"><h2>Read the background.</h2><div><p>Sources provided by the organizer.</p><ol class="sources">${p.sources
          .filter((s) => s.url)
          .map(
            (s) =>
              `<li><a href="${esc(s.url)}" rel="noopener noreferrer" target="_blank">${esc(s.title || s.url)}</a></li>`,
          )
          .join("")}</ol></div></section>`
      : ""
  }
...
export async function buildFiles(p, url) {
  const revision = await artifactRevision(p);
  const [html, pdf, social, story, svg] = await Promise.all([
    siteHTML(p, url, revision),
    posterPDF(p, url),
    posterPNG(p, url, "square"),
    posterPNG(p, url, "story"),
    posterSVG(p, url, p.format),
  ]);
  return {
    revision,
    files: {
      ...
      "flier.pdf": pdf,
      "flier.svg": Buffer.from(svg),
      "social.png": social,
      "story.png": story,
      "event.ics": Buffer.from(calendar(p, url)),
      "webmcp.js": Buffer.from(publicWebMCP),
      ".nojekyll": Buffer.from(""),
      "protest-public.json": Buffer.from(
        JSON.stringify(publicProject(p), null, 2),
      ),
    },
  };
}
